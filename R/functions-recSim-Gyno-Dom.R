					...
					if(hm == 0.5 & hf == 0.5) {
				   	...
				   	if(par.list$C == 0) {
						...
						Gii.init  <-  rep(0,10)
				   		...
				   		QEs   <-  QEs/sum(QEs)
				   	   	QEs[QEs == max(QEs)]  <-  max(QEs) - 0.01
						Fii.init  <-  round(c((1 - par.list$C)*QEs[1], 0, (1 - par.list$C)*QEs[2], 0, 0, (1 - par.list$C)*0.01, 0, (1 - par.list$C)*QEs[3], 0, 0), digits=5)
						Gii.init  <-  round(c(par.list$C*QEs[1], 0, par.list$C*QEs[2], 0, 0, par.list$C*0.01, 0, par.list$C*QEs[3], 0, 0), digits=5)
					}
...
#        lines(femRelFreq[k==ks[4]] ~ C[k==ks[4]], lwd=2, col=COLS[4], cex=1, data=dat)
        # axes
